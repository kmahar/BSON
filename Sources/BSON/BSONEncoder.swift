...
internal class _BSONEncoder: Encoder {
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        let topContainer: BSONMutableArray
        if self.canEncodeNewValue {
            // We haven't yet pushed a container at this level; do so here.
            topContainer = self.storage.pushUnkeyedContainer()
        } else {
            guard let container = self.storage.containers.last as? BSONMutableArray else {
                fatalError(
...
internal struct _BSONEncodingStorage {
    ...
    fileprivate mutating func pushUnkeyedContainer() -> BSONMutableArray {
        let array = BSONMutableArray()
...
private class _BSONReferencingEncoder: _BSONEncoder {
    ...
    private enum Reference {
        ...
        case array(BSONMutableArray, Int)
    ...
    fileprivate init(referencing encoder: _BSONEncoder, at index: Int, wrapping array: BSONMutableArray) {
...
private struct _BSONUnkeyedEncodingContainer: UnkeyedEncodingContainer {
    ...
    /// A reference to the container we're writing to.
    private let container: BSONMutableArray
    ...
    /// Initializes `self` with the given references.
    fileprivate init(referencing encoder: _BSONEncoder, codingPath: [CodingKey], wrapping container: BSONMutableArray) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
...
private class BSONMutableArray: BSONValue {
...
private class MutableDictionary: BSONValue {
    ...
    internal required init(from data: inout Data) throws {
        throw InternalError(message: "mutable dict init")
    }
