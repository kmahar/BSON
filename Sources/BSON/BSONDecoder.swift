...
public class BSONDecoder {
    ...
    public func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        var data = data
        return try self.decode(type, from: try Document(from: &data))
    }
...
//        }
     ...
     *
     * - Throws:
     *   - `InternalError` if the BSON data is corrupt or if any value throws an error during decoding.
     */
    ...
    throws -> T {
        do {
            ...
            throw InternalError(message: errMsg)
        }
    }
...
private struct _BSONKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    private func _superDecoder(forKey key: CodingKey) throws -> Decoder {
        return try self.decoder.with(pushedKey: key) {
            ...
            return _BSONDecoder(referencing: value.bsonValue, at: self.decoder.codingPath, options: self.decoder.options)
        }
    }
